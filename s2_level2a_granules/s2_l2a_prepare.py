...
def prepare_dataset(path):
    ...
    granuleslist = [(granule.get('granuleIdentifier'), [imid.text for imid in granule.findall('IMAGE_FILE')]) for
                    granule in
                    root.findall('./*/Product_Info/Product_Organisation/Granule_List/Granule')]
    granules = {}
    ...
    for granule_id, images in granules.items():
        ...
        gran_path = str(path.parent.joinpath(Path(images[0]).parent.parent.parent, 'MTD_TL' + '.xml'))

        root = ElementTree.parse(gran_path).getroot()
        sensing_time = root.findall('./*/SENSING_TIME')[0].text
        #img_data_path = str(path.parent.joinpath(Path(images[0]).parent.parent.parent, 'IMG_DATA'))
        #img_data_path_r10 = str(path.parent.joinpath(Path(images[0]).parent.parent.parent, 'IMG_DATA', 'R10m'))
        #img_data_path_r20 = str(path.parent.joinpath(Path(images[0]).parent.parent.parent, 'IMG_DATA', 'R20m'))
        #img_data_path_r60 = str(path.parent.joinpath(Path(images[0]).parent.parent.parent, 'IMG_DATA', 'R60m'))
        ...
        for image in images:
            ...
            for item in classification_list:
                if item in image:
                    ...
                    if '20m' in image:
                        images_classification.append(str(path.parent.joinpath(image + ".jp2")))

            for item in ten_list:
                if item in image:
                    images_ten_list.append(str(path.parent.joinpath(image + ".jp2")))
                    grouped_images.append(str(path.parent.joinpath(image + ".jp2")))
            for item in twenty_list:
                if item in image:
                    images_twenty_list.append(str(path.parent.joinpath(image + ".jp2")))
                    grouped_images.append(str(path.parent.joinpath(image + ".jp2")))
            for item in sixty_list:
                if item in image:
                    images_sixty_list.append(str(path.parent.joinpath(image + ".jp2")))
                    grouped_images.append(str(path.parent.joinpath(image + ".jp2")))
        ...
        res=spatial_ref.SetFromUserInput(cs_code)
        if res != 0:
            raise RuntimeError(repr(res) + ': could not import from '+cs_code+' Is %GDAL_DATA% defined?')
...
@click.command(
    help="Prepare Sentinel 2 L2 sen2cor dataset SR and SC for ingestion into the Data Cube. "
         "eg. python s2_l2a_prepare.py <input>.SAFE --output <outfile>.yaml")
